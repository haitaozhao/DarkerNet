...
int resize_network(network *net, int w, int h)
{
    ...
    for (i = 0; i < net->n; ++i){
        ...
        // 2019/5/6
            ...
            resize_connected_layer(&l);
        // 2019/5/7
        }else if(l.type == PRELU){
            resize_prelu_layer(&l, w, h);
...
void print_network(network *net)
{
    ...
    for(i = 0; i < net->n; ++i){
        ...
        int n = l.outputs;
        fprintf(stderr, "Layer %d, input  %dx%dx%d, output %dx%dx%d\n", i, l.h, l.w, l.c, l.out_h, l.out_w, l.out_c);

        float mean = mean_array(output, n);
        float vari = variance_array(output, n);
        fprintf(stderr, "Mean: %f, Variance: %f\n", mean, vari);
...
void print_convolutional_weights(layer l)
{
    int num = l.nweights;
    fprintf(stderr, "bias:    ");
    print_memory(l.biases, l.n);
    if (l.batch_normalize){
        fprintf(stderr, "batchnorm: ");
        print_memory(l.scales, l.n);
        print_memory(l.rolling_mean, l.n);
        print_memory(l.rolling_variance, l.n);
    }
    fprintf(stderr, "weights: ");
    print_memory(l.weights, num);
}

void print_connected_weights(layer l)
{
    fprintf(stderr, "bias:    ");
    print_memory(l.biases, l.outputs);
    fprintf(stderr, "weights: ");
    print_memory(l.weights, l.outputs*l.inputs);
    if (l.batch_normalize){
        fprintf(stderr, "batchnorm: ");
        print_memory(l.biases, l.outputs);
        print_memory(l.biases, l.outputs);
...
void print_batchnorm_weights(layer l)
{
    print_memory(l.scales, l.c);
    print_memory(l.rolling_mean, l.c);
    print_memory(l.rolling_variance, l.c);
}

void print_network_weights(network *net, int cutoff)
{
    int i;
    for(i = 0; i < net->n && i < cutoff; ++i){

        layer l = net->layers[i];
        fprintf(stderr, "layer: [%d]/[%d], type: %s\n", i+1, net->n, layer_type_to_string(l.type));

        if (l.dontsave) continue;
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            print_convolutional_weights(l);
        } if(l.type == CONNECTED){
            print_connected_weights(l);
        } if(l.type == BATCHNORM){
            print_batchnorm_weights(l);
        } if(l.type == RNN){
            print_connected_weights(*(l.input_layer));
            print_connected_weights(*(l.self_layer));
            print_connected_weights(*(l.output_layer));
        } if (l.type == LSTM) {
            print_connected_weights(*(l.wi));
            print_connected_weights(*(l.wf));
            print_connected_weights(*(l.wo));
            print_connected_weights(*(l.wg));
            print_connected_weights(*(l.ui));
            print_connected_weights(*(l.uf));
            print_connected_weights(*(l.uo));
            print_connected_weights(*(l.ug));
        } if (l.type == GRU) {
            if(1){
                print_connected_weights(*(l.wz));
                print_connected_weights(*(l.wr));
                print_connected_weights(*(l.wh));
                print_connected_weights(*(l.uz));
                print_connected_weights(*(l.ur));
                print_connected_weights(*(l.uh));
            }else{
                print_connected_weights(*(l.reset_layer));
                print_connected_weights(*(l.update_layer));
                print_connected_weights(*(l.state_layer));
            }
        } if(l.type == CRNN){
            print_convolutional_weights(*(l.input_layer));
            print_convolutional_weights(*(l.self_layer));
            print_convolutional_weights(*(l.output_layer));
        } if(l.type == LOCAL){
            int locations = l.out_w*l.out_h;
            int size = l.size*l.size*l.c*l.n*locations;
            print_memory(l.biases, l.outputs);
            print_memory(l.weights, size);
        } if(l.type == PRELU){
            print_memory(l.weights, l.n);
        }
